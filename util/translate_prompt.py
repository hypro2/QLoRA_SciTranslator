...
def prompt_chat_completion(dialog, system_token="### System:", user_token="### Instruction:",
                           assistant_token="### Response:", start_token="", end_token=""):
...
{start_token}{(answer['value']).strip()}{end_token}</s>"""
    ...
    if '다음 문장을 영어로 번역하라:' in dialog_tokens:
        dialog_tokens = dialog_tokens.replace('다음 문장을 영어로 번역하라:', "")
